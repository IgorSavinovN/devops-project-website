    ...
    return jsonify({
        ...
        "visits": {
            ...
            "today": Visit.query.filter(
                Visit.timestamp >= datetime.utcnow().replace(hour=0, minute=0, second=0)).count()
        },
        "timestamp": datetime.utcnow().isoformat()
    })
...
if __name__ == '__main__':
    with app.app_context():
        ...
        if Project.query.count() == 0:
            test_projects = [
                Project(title="Настроить CI/CD", description="GitHub Actions автоматизация", priority="high",
                        status="in_progress"),
